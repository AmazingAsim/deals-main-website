...
export default function AmazonCard({ product }) {
  ...
  const {
    ...
    description, image, asin, created_at
  } = product;
  ...
  const timeInEst = new Date(created_at).toLocaleString('en-US', { timeZone: 'America/New_York' });
    let imagBaseUrl = 'https://dealsfromamerica.com/employee/uploads/products';
  return (
    <div className="card mb-3 shadow-sm" style={{cursor:'pointer'}} onClick={()=>{navigate(`/product-details/${product.asin?product.asin:product.id}/amazon`)}} >
        ...
        {/* Product Details Section */}
          <div className="card-body">
            ...
            {
              selling_price && <div>
                <p className='fs-4'><b className='text-success'>{formattedDiscount!="NaN" && formattedDiscount!=0.00?`-${formattedDiscount}%`:''}</b> 
                <i className='ms-3'>{currency==='INR'?'₹':'$'}{selling_price}</i></p>
            <p className='text-muted fs-6'><small>MRP: <s>{currency==='INR'?'₹':'$'}{original_price}</s></small></p>
              </div>
            }
            <p className="card-text">
              <small className="text-muted">Store: Amazon.com</small><br/>
