...
export default function AmazonCatagory(props) {
  let {catagory} = props;
  ...
  async function fetchProducts(newPage = page) {
    ...
    setLoading(true);
    try {
      let res = await fetch(`${fetchUrl}?&limit=50&page=${newPage}&category=${catagory}`,{method:"GET",mode:"cors"});
